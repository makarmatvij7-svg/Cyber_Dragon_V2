...
local function RunCyberDragon()
    ...
    local AutoLoad = {}
    AutoLoad.ConfigFile = "CyberDragon/autoload.json"

    function AutoLoad:LoadConfig()
        if not readfile or not isfile then return nil end
        if not isfile(self.ConfigFile) then return nil end
        local success, data = pcall(readfile, self.ConfigFile)
    ...
    function AutoLoad:SaveConfig(config)
        if not writefile then return end
        pcall(function()
            makefolder("CyberDragon")
    ...
    function AutoLoad:GetDefaultConfig()
        return {
            AutoEnabled = {
                NoRecoil = false,
                NoSpread = false,
    ...
    function AutoLoad:ApplyConfig(config)
        ...
        if config.Settings then
            ...
            if config.Settings.ESPMaxDistance then
                espSettings.MaxDistance = config.Settings.ESPMaxDistance
            end
            if config.Settings.ESPBoxType then
                espSettings.BoxType = config.Settings.ESPBoxType
            end
            if config.Settings.ESPTracerOrigin then
                espSettings.TracerOrigin = config.Settings.ESPTracerOrigin
            end
            if config.Settings.ShowTracers ~= nil then
                espSettings.ShowTracers = config.Settings.ShowTracers
            end
            if config.Settings.ShowChams ~= nil then
                espSettings.ShowChams = config.Settings.ShowChams
            end
            if config.Settings.ShowWeapon ~= nil then
                espSettings.ShowWeapon = config.Settings.ShowWeapon
            end
            if config.Settings.ShowSkeleton ~= nil then
                espSettings.ShowSkeleton = config.Settings.ShowSkeleton
            end
            if config.Settings.TeamCheck ~= nil then
                espSettings.TeamCheck = config.Settings.TeamCheck
            end
            if config.Settings.FarmPosition then
                farmPosition = config.Settings.FarmPosition
            end
            if config.Settings.DesyncDepth then
                DESYNC_CONFIG.DESYNC_DEPTH = -config.Settings.DesyncDepth
            end
            if config.Settings.ShootDelay then
                DESYNC_CONFIG.SHOOT_DESYNC_DELAY = config.Settings.ShootDelay
            end
            if config.Settings.PacketDelay then
                DESYNC_CONFIG.PACKET_DELAY = config.Settings.PacketDelay
            end
            if config.Settings.HeadOffset then
                DESYNC_CONFIG.HEAD_OFFSET_Y = config.Settings.HeadOffset
            end
        end

        -- Apply walk speed and jump power immediately
        task.spawn(function()
            task.wait(1)
        ...
        return config.AutoEnabled or {}
    end

    function AutoLoad:EnableFeatures(autoEnabled)
        if not autoEnabled then return end

        task.spawn(function()
            local delayTime = self.Config and self.Config.AutoLoadDelay or 3
            task.wait(delayTime)

            -- Combat features
            if autoEnabled.NoRecoil then
                state.NoRecoil = true
                startWeaponMods()
                if Library then Library:Notify("Auto-enabled: No Recoil", 2) end
            end
            if autoEnabled.NoSpread then
                state.NoSpread = true
                startWeaponMods()
                if Library then Library:Notify("Auto-enabled: No Spread", 2) end
            end
            if autoEnabled.RapidFire then
                state.RapidFire = true
                startWeaponMods()
                if Library then Library:Notify("Auto-enabled: Rapid Fire", 2) end
            end
            if autoEnabled.InstantScope then
                state.InstantScope = true
                startWeaponMods()
                if Library then Library:Notify("Auto-enabled: Instant Scope", 2) end
            end
            if autoEnabled.AutoWeapon then
                state.AutoWeapon = true
                enableAutoWeapon()
                if Library then Library:Notify("Auto-enabled: Auto Weapon", 2) end
            end
            if autoEnabled.AlwaysBackstab then
                state.AlwaysBackstab = true
                for _, p in pairs(Players:GetPlayers()) do
                    if p.Character then applyAlwaysBackstab(p.Character) end
                end
                if Library then Library:Notify("Auto-enabled: Always Backstab", 2) end
            end
            if autoEnabled.AntiKatana then
                state.AntiKatana = true
                enableAntiKatana()
                if Library then Library:Notify("Auto-enabled: Anti Katana", 2) end
            end
            if autoEnabled.AutoDrop then
                state.AutoDrop = true
                if Library then Library:Notify("Auto-enabled: Auto Drop", 2) end
            end
            if autoEnabled.AutoFarm then
                state.AutoFarm = true
                enableAutoFarm()
                if Library then Library:Notify("Auto-enabled: Auto Farm", 2) end
            end

            -- Movement features
            if autoEnabled.Fly then
                state.Fly = true
                enableFly()
                if Library then Library:Notify("Auto-enabled: Fly", 2) end
            end
            if autoEnabled.Noclip then
                state.Noclip = true
                enableNoclip()
                if Library then Library:Notify("Auto-enabled: Noclip", 2) end
            end
            if autoEnabled.JumpBug then
                state.JumpBug = true
                enableJumpBug()
                if Library then Library:Notify("Auto-enabled: Jump Bug", 2) end
            end
            if autoEnabled.AutoStrafe then
                state.AutoStrafe = true
                enableAutoStrafe()
                if Library then Library:Notify("Auto-enabled: Auto Strafe", 2) end
            end
            if autoEnabled.AntiAim then
                state.AntiAim = true
                enableAntiAim()
                if Library then Library:Notify("Auto-enabled: Anti Aim", 2) end
            end
            if autoEnabled.TornadoAnim then
                _state.TornadoAnim = true
                _enableTornadoAnim()
                if Library then Library:Notify("Auto-enabled: Tornado Animation", 2) end
            end

            -- Visual features
            if autoEnabled.ESP then
                state.ESP = true
                if Library then Library:Notify("Auto-enabled: ESP", 2) end
            end
            if autoEnabled.ThirdPerson then
                state.ThirdPerson = true
                enableThirdPerson()
                if Library then Library:Notify("Auto-enabled: Third Person", 2) end
            end
            if autoEnabled.HitNotif then
                state.HitNotif = true
                if Library then Library:Notify("Auto-enabled: Hit Notifications", 2) end
            end

            -- World features
            if autoEnabled.NoBounds then
                state.NoBounds = true
                enableNoBounds()
                if Library then Library:Notify("Auto-enabled: Prevent OOB", 2) end
            end
            if autoEnabled.RemoveKillers then
                state.RemoveKillers = true
                enableRemoveKillers()
                if Library then Library:Notify("Auto-enabled: Remove Killers", 2) end
            end
            if autoEnabled.NoFireDamage then
                state.NoFireDamage = true
                enableNoFireDamage()
                if Library then Library:Notify("Auto-enabled: No Fire Damage", 2) end
            end
            if autoEnabled.AntiFreeze then
                state.AntiFreeze = true
                enableAntiFreeze()
                if Library then Library:Notify("Auto-enabled: Anti Freeze", 2) end
            end

            -- Desync + Wallbang
            if autoEnabled.DesyncWallbang then
                DesyncWallbang.Active = true
                task.spawn(function()
                    pcall(function() DesyncWallbang:Init() end)
                end)
                if Library then Library:Notify("Auto-enabled: Desync + Wallbang", 2) end
            end

            -- Unlock Cosmetics
            if autoEnabled.UnlockCosmetics then
                state.UnlockCosmetics = true
                task.spawn(function()
                    pcall(UnlockAll)
                    if getgenv()._CyberDragon_unlockRan then
                        if Library then Library:Notify("Auto-enabled: All Skins unlocked!", 3) end
                    end
                end)
            end
        end)
    ...
    AutoLoadGroup:AddToggle("AutoExecute", {
        Text = "Auto-Execute Script",
        Default = false,
        Callback = function(Value)
            local config = AutoLoad:LoadConfig() or AutoLoad:GetDefaultConfig()
            config.AutoExecute = Value
            AutoLoad:SaveConfig(config)
        end
    })

    AutoLoadGroup:AddSlider("AutoLoadDelay", {
        Text = "Auto-Load Delay (sec)",
        Default = 3,
        Min = 0,
        Max = 10,
        Rounding = 0,
        Callback = function(Value)
            local config = AutoLoad:LoadConfig() or AutoLoad:GetDefaultConfig()
            config.AutoLoadDelay = Value
            AutoLoad:SaveConfig(config)
        end
    })

    AutoLoadGroup:AddLabel("Features to Auto-Enable:", true)

    AutoLoadGroup:AddToggle("AutoNoRecoil", {
        Text = "Auto: No Recoil",
        Default = false,
        Callback = function(Value)
            local config = AutoLoad:LoadConfig() or AutoLoad:GetDefaultConfig()
            config.AutoEnabled = config.AutoEnabled or {}
            config.AutoEnabled.NoRecoil = Value
            AutoLoad:SaveConfig(config)
        end
    })

    AutoLoadGroup:AddToggle("AutoNoSpread", {
        Text = "Auto: No Spread",
        Default = false,
        Callback = function(Value)
            local config = AutoLoad:LoadConfig() or AutoLoad:GetDefaultConfig()
            config.AutoEnabled = config.AutoEnabled or {}
            config.AutoEnabled.NoSpread = Value
            AutoLoad:SaveConfig(config)
        end
    })

    AutoLoadGroup:AddToggle("AutoRapidFire", {
        Text = "Auto: Rapid Fire",
        Default = false,
        Callback = function(Value)
            local config = AutoLoad:LoadConfig() or AutoLoad:GetDefaultConfig()
            config.AutoEnabled = config.AutoEnabled or {}
            config.AutoEnabled.RapidFire = Value
            AutoLoad:SaveConfig(config)
        end
    })

    AutoLoadGroup:AddToggle("AutoESP", {
        Text = "Auto: ESP",
        Default = false,
        Callback = function(Value)
            local config = AutoLoad:LoadConfig() or AutoLoad:GetDefaultConfig()
            config.AutoEnabled = config.AutoEnabled or {}
            config.AutoEnabled.ESP = Value
            AutoLoad:SaveConfig(config)
        end
    })

    AutoLoadGroup:AddToggle("AutoFly", {
        Text = "Auto: Fly",
        Default = false,
        Callback = function(Value)
            local config = AutoLoad:LoadConfig() or AutoLoad:GetDefaultConfig()
            config.AutoEnabled = config.AutoEnabled or {}
            config.AutoEnabled.Fly = Value
            AutoLoad:SaveConfig(config)
        end
    })

    AutoLoadGroup:AddToggle("AutoNoclip", {
        Text = "Auto: Noclip",
        Default = false,
        Callback = function(Value)
            local config = AutoLoad:LoadConfig() or AutoLoad:GetDefaultConfig()
            config.AutoEnabled = config.AutoEnabled or {}
            config.AutoEnabled.Noclip = Value
            AutoLoad:SaveConfig(config)
        end
    })

    AutoLoadGroup:AddToggle("AutoHitNotif", {
        Text = "Auto: Hit Notifications",
        Default = true,
        Callback = function(Value)
            local config = AutoLoad:LoadConfig() or AutoLoad:GetDefaultConfig()
            config.AutoEnabled = config.AutoEnabled or {}
            config.AutoEnabled.HitNotif = Value
            AutoLoad:SaveConfig(config)
        end
    })

    AutoLoadGroup:AddToggle("AutoDesync", {
        Text = "Auto: Desync + Wallbang",
        Default = false,
        Callback = function(Value)
            local config = AutoLoad:LoadConfig() or AutoLoad:GetDefaultConfig()
            config.AutoEnabled = config.AutoEnabled or {}
            config.AutoEnabled.DesyncWallbang = Value
            AutoLoad:SaveConfig(config)
        end
    })
    ...
    AutoLoadGroup:AddButton({
        ...
        Func = function()
            ...
            config.Settings.WalkSpeed = settings.WalkSpeed
            config.Settings.JumpPower = settings.JumpPower
            config.Settings.StrafeIntensity = settings.StrafeIntensity
            config.Settings.FlySpeed = settings.FlySpeed
            config.Settings.TornadoAnimSpeed = settings.TornadoAnimSpeed
            config.Settings.ESPMaxDistance = espSettings.MaxDistance
            config.Settings.ESPBoxType = espSettings.BoxType
            config.Settings.ESPTracerOrigin = espSettings.TracerOrigin
            config.Settings.ShowTracers = espSettings.ShowTracers
            config.Settings.ShowChams = espSettings.ShowChams
            config.Settings.ShowWeapon = espSettings.ShowWeapon
            config.Settings.ShowSkeleton = espSettings.ShowSkeleton
            config.Settings.TeamCheck = espSettings.TeamCheck
            config.Settings.FarmPosition = farmPosition
            config.Settings.DesyncDepth = math.abs(DESYNC_CONFIG.DESYNC_DEPTH)
            config.Settings.ShootDelay = DESYNC_CONFIG.SHOOT_DESYNC_DELAY
            config.Settings.PacketDelay = DESYNC_CONFIG.PACKET_DELAY
            config.Settings.HeadOffset = DESYNC_CONFIG.HEAD_OFFSET_Y
            AutoLoad:SaveConfig(config)
            Library:Notify("Auto-Load settings saved!", 3)
        end,
        DoubleClick = false,
        Tooltip = "Saves current feature settings for auto-load"
    })

    AutoLoadGroup:AddButton({
        Text = "Reset Auto-Load Config",
        Func = function()
            local defaultConfig = AutoLoad:GetDefaultConfig()
            AutoLoad:SaveConfig(defaultConfig)
            Library:Notify("Auto-Load config reset to defaults!", 3)
        end,
        DoubleClick = true,
        Tooltip = "Resets auto-load settings to default (double-click)"
    })
    ...
    -- ========== AUTO-LOAD EXECUTION ==========
    task.spawn(function()
        local config = AutoLoad:LoadConfig()
        if config and config.AutoExecute then
            -- Update config with current place
            config.LastPlaceId = game.PlaceId
            AutoLoad:SaveConfig(config)

            -- Apply settings and enable features
            local autoEnabled = AutoLoad:ApplyConfig(config)
            AutoLoad:EnableFeatures(autoEnabled)

            if Library then
                Library:Notify("Auto-Load: Applying saved configuration...", 3)
            end
        else
            -- First time or auto-execute disabled - save default config
            if not config then
                local defaultConfig = AutoLoad:GetDefaultConfig()
                defaultConfig.LastPlaceId = game.PlaceId
                AutoLoad:SaveConfig(defaultConfig)
            end
        end
    end)
...
if not getgenv()._CyberDragon_KeyValid then
    ...
    local autoLoadFile = "CyberDragon/autoload.json"
    local shouldAutoExecute = false
    local autoLoadDelay = 0
    ...
    pcall(function()
        if readfile and isfile and isfile(autoLoadFile) then
            ...
            if config and config.AutoExecute then
                ...
                autoLoadDelay = config.AutoLoadDelay or 0
            end
        end
    end)

    if shouldAutoExecute and autoLoadDelay > 0 then
        print("[Cyber Dragon] Auto-execute enabled, waiting " .. autoLoadDelay .. "s...")
